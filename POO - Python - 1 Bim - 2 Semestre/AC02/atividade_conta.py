...
class Conta:
	...
	def depositar(self, valor):
		if (self.ativa is True and valor > 0):
			self.__saldo += valor
			self.__operacoes.append(('deposito', valor))

	def sacar(self, valor):
		if self.ativa is True and valor > 0 and valor <= self.__saldo:
			self.__saldo -= valor
			self.__operacoes.append(('saque', valor))

	def transferir(self, conta_destino, valor):
		if self.__ativa == True and valor > 0 and conta_destino.ativa == True and valor <= self.__saldo:
			self.__saldo -= valor
